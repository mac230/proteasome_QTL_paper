## -----
## data are taken from the 1,011 genomes project:
## paper: https://doi.org/10.1038/s41586-018-0030-5
## data: http://1002genomes.u-strasbg.fr/files/


## -----
## setup
## load the big, or a reduced VCF, call it "af_data"
base_dir <- "~/emacs/proteasome_QTL_paper/scripts_data/variant_population_frequency/"
load(paste0(base_dir, "PeterGenotypesRMAlt_200505.RData"))

## simplify the name for subsequent work
af_data <- PeterGenotypesRMAlt

## make a table with the variant's chromosome and position
variants <- data.frame(Position = 411461,
                       Chr = "chrVII",
                       Gene = "RPT6",
                       Causal = NA)
